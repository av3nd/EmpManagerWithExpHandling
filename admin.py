...
def update():
    ...
    try:
        ...
        messagebox.showinfo("Error", "Enter Sno. in the Employee ID entry box, to update a record")
        root.destroy()
        os.system('admin.py')
...
def delete():
    if (employeeID.get()==""):
        messagebox.showinfo("Error","Please enter Sno in Employee ID to delete a record")
    else:
        root.withdraw()
        conn = sqlite3.connect('EmployeeInfo.db')
        c = conn.cursor()
